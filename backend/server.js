...
wssTerminal.on('connection', (ws) => {
  ...
  ws.send(`Workspace: \x1b[33m${currentWorkspace}\x1b[0m\r\n`);
  ws.send('Type commands below. Connected to backend server.\r\n\r\n');
  const getPrompt = () => `\r\n\x1b[1;31m(renkairo)\x1b[0m \x1b[1;34mdeveloper@Renkairo\x1b[0m \x1b[33m${path.basename(currentWorkspace)} %\x1b[0m `;
  ws.send(getPrompt());
      ...
      if (char === '\r' || char === '\n') {
        ...
        if (cmd) {
          if (cmd === 'clear' || cmd === 'cls') {
            ...
            const proc = spawn(shell, [process.platform === 'win32' ? '-Command' : '-c', cmd], { cwd: currentWorkspace });
