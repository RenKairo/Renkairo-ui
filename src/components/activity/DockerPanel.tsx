  ...
  return (
    <aside className="w-full bg-[var(--bg-panel)] border-r border-[var(--border-color)] flex flex-col select-none h-full z-10 font-sans transition-colors duration-150">
      <div className="h-9 px-3 border-b border-[var(--border-color)] flex items-center justify-between text-xs text-[var(--text-muted)] font-semibold uppercase tracking-wider bg-[var(--bg-panel)]">
        ...
        <button title="Refresh Docker Engine" className="p-1 hover:text-[var(--text-primary)] transition-colors">
            ...
            <div key={c.id} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg p-2.5 space-y-2 shadow-sm">
              <div className="flex items-start justify-between">
                <div className="flex items-center space-x-2">
                  <Box className="w-4 h-4 text-[var(--accent-cyan)] shrink-0" />
                  <div>
                    <h4 className="text-xs font-semibold text-[var(--text-primary)]">{c.name}</h4>
                    <span className="text-[9px] text-[var(--text-muted)]">{c.image}</span>
                  </div>
                </div>

                <span className={`text-[9px] px-1.5 py-0.5 rounded ${
                  c.status === 'running' ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/30' : 'bg-[var(--bg-panel)] text-[var(--text-muted)] border border-[var(--border-color)]'
                }`}>
                  {c.status}
                </span>
              </div>

              <div className="flex items-center justify-between pt-1 border-t border-[var(--border-color)] text-[10px] text-[var(--text-muted)]">
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <button 
                    ...
                    className={`p-1 ${c.status === 'running' ? 'text-rose-500 hover:text-rose-600' : 'text-emerald-500 hover:text-emerald-600'}`}
                  >
                    {c.status === 'running' ? <Square className="w-3 h-3 fill-rose-500" /> : <Play className="w-3 h-3 fill-emerald-500" />}
                  </button>
                </div>
              </div>
            </div>
